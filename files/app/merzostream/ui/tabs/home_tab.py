...
from ...core.content import load_dashboard, load_theme
from ...core.paths import CLIENT_SECRET, KICK_TOKEN, YOUTUBE_TOKEN


class HomeTab(ctk.CTkScrollableFrame):
    def __init__(self, parent, app_cfg, stream_cfg, player_cfg):
        ...
        note = ctk.CTkFrame(self, fg_color="#1f2c3a", corner_radius=12)
        note.grid(row=notice_row, column=0, columnspan=2, sticky="ew", padx=10, pady=(18, 10))
        ctk.CTkLabel(note, text=dashboard.get("notice", ""), font=("Arial", 13, "bold"), text_color="#dbeafe", wraplength=900, justify="left").pack(anchor="w", padx=18, pady=18)
