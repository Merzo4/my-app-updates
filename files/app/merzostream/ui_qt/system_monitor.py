...
class MetricCard(QFrame):
    def __init__(self, title: str):
        ...
        self.setToolTip("Карточка Dashboard. Показывает текущее состояние этого показателя и обновляется автоматически.")
        self.value = QLabel("—")
...
class SystemMonitorPanel(QFrame):
    def __init__(self, theme: dict):
        ...
        self.settings_button.setToolTip("Настрой Dashboard: CPU, RAM, сеть, каждый диск, каждый GPU, платформы, медиаплеер и фоновая музыка.")
        ...
        self.host.setAttribute(Qt.WA_TranslucentBackground, True)
    ...
    def _rebuild_cards(self):
        ...
        if not self.cards:
            empty = QLabel("Все показатели скрыты. Нажми ⚙ и включи нужные блоки.")
            empty.setWordWrap(True); empty.setObjectName("metricTitle")
            empty.setToolTip("Открой настройки Dashboard по кнопке ⚙ и выбери нужные показатели.")
