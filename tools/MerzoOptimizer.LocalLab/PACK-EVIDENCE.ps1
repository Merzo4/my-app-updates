...
try{
  ...
  $tmp=Join-Path $root 'Temp\Evidence'
  ...
  Write-Host "EVIDENCE_READY $out sha256=$sha"
  if(Test-Path $autoReport){try{& pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File $autoReport -Event 'utility.pack-evidence' -Outcome PASS -Message "Evidence ZIP ready sha256=$sha" -LogPath $log|Out-Null}catch{}}
  Start-Process explorer.exe "/select,`"$out`""
  exit 0
}catch{
  $message=$_.Exception.Message
  try{Add-Content $log "[pack-evidence] FAIL: $message" -Encoding UTF8}catch{}
  if(Test-Path $autoReport){try{& pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File $autoReport -Event 'utility.pack-evidence' -Outcome FAIL -Message $message -LogPath $log|Out-Null}catch{}}
  Write-Error $message
  exit 1
}
