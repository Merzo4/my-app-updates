...
if([long]$ci.databaseId -ne 32232868999 -or $ci.headSha -ne 'c8ed6d8bdaca6ef7178f8876379821bc3c16ed23'){throw "R54.1 CI status is not the exact approved green run: run=$($ci.databaseId) head=$($ci.headSha)"}
if($runtime.conclusion -ne 'success' -or [long]$runtime.buildRun -ne 32232868999 -or $runtime.applyViaProductScm -ne 'success' -or $runtime.restoreViaProductScm -ne 'success' -or $runtime.launch -ne 'success'){throw 'R54.1 TrkWks runtime acceptance is not green'}
