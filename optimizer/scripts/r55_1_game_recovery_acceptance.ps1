...
$old='$fv-ne''0.1.54.2'''
$new='$fv-ne''0.1.55.1'''
if(($patched.Split($old).Count-1)-ne1){throw 'R55.1 GAME version assertion anchor mismatch'}
...
try{
  ...
  if($LASTEXITCODE-ne0){throw "R55.1 inherited GAME/RestoreAll v12 failed: $LASTEXITCODE"}
  $legacy='.\optimizer\R54_2_GAME_MUTATION_STATUS.json'
  if(!(Test-Path $legacy)){throw 'R55.1 inherited GAME status missing'}
  ...
  foreach($name in @('oneDriveSyntheticNonzero','oneDriveSetupOnlyDetection','gameFullMutation','recoveryRestoreAll','recoveryStateVerification')){
    if($j.$name-ne'success'){throw "R55.1 GAME/Recovery field not green: $name=$($j.$name)"}
  }
  $portable=Get-ChildItem $ArtifactDir -Recurse -File -Filter 'MerzoWindowsOptimizer-portable-win-x64.zip'|Select-Object -First 1
  if(!$portable){throw 'R55.1 portable missing'}
  $portableSha=(Get-FileHash $portable.FullName -Algorithm SHA256).Hash.ToLowerInvariant()
  if($portableSha-ne$ExpectedPortableSha.ToLowerInvariant()){throw "R55.1 portable SHA mismatch $portableSha"}
  $work=Join-Path $env:RUNNER_TEMP ('r551-version-'+[guid]::NewGuid().ToString('N'));Expand-Archive $portable.FullName $work -Force
  $fv=(Get-Item (Join-Path $work 'MerzoWindowsOptimizer.exe')).VersionInfo.FileVersion
  if($fv-ne'0.1.55.1'){throw "R55.1 final GAME version=$fv"}
  [ordered]@{
    ...
    oneDriveSyntheticNonzero='success';oneDriveSetupOnlyDetection='success';gameFullMutation='success';undoContractProbe='production-restore-all-success';recoveryRestoreAll='success';recoveryStateVerification='success'
  }|ConvertTo-Json|Set-Content '.\optimizer\R55_1_GAME_RECOVERY_STATUS.json' -Encoding UTF8
  Write-Host "R55_1_GAME_RECOVERY_ACCEPTANCE_PASS buildRun=$BuildRun version=$fv portableSha=$portableSha"
