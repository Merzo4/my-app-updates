...
var configuredProperty=result.GetType().GetProperty("IsConfigured");
var configuredText=configuredProperty is null?"n/a":configuredProperty.GetValue(result)?.ToString()??"null";
Console.WriteLine($"R54_REAL_CHECK success={B("Success")} configured={configuredText} available={B("UpdateAvailable")} current={S("CurrentVersion")} latest={S("LatestVersion")}");
Console.WriteLine($"R54_REAL_ASSET={S("AssetUrl")}");
Console.WriteLine($"R54_REAL_CHECKSUM={S("ChecksumUrl")}");
if(!B("Success")||!B("UpdateAvailable")) throw new Exception("R54 CheckAsync did not expose an available update");
