...
var check=type.GetMethod("CheckAsync") ?? throw new Exception("CheckAsync missing");
var checkTask=(Task)check.Invoke(service,null)!;
await checkTask;
...
var download=type.GetMethod("DownloadAsync") ?? throw new Exception("DownloadAsync missing");
object?[]? da=null;
foreach(var m in type.GetMethods().Where(m=>m.Name=="DownloadAsync")){
  ...
  if(ps.Length==2 && ps[0].ParameterType.IsInstanceOfType(result)){download=m;da=new object?[]{result,CancellationToken.None};break;}
}
if(da is null) throw new Exception("Compatible DownloadAsync overload missing");
