...
parts=[patch_dir/"r46_payload.part00",patch_dir/"r46_payload.part01"]+[patch_dir/f"r46_payload.chunk{i:02d}" for i in range(6,19)]
missing=[p.name for p in parts if not p.is_file()]
if missing: raise SystemExit("R46 payload parts missing: "+", ".join(missing))
