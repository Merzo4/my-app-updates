...
# -----------------------------------------------------------------------------
# R35 READABILITY UI
# Fix the cramped/high-DPI look reported on a 2K monitor without making the
# compact 1000x600 shell large.  We keep the teal identity but reduce neon
# saturation, give buttons real breathing room and round layout to device pixels.
# -----------------------------------------------------------------------------
app_path = root / 'src' / 'MerzoOptimizer.App' / 'App.xaml'
app = read(app_path)

# Softer brand accent: still recognisably Merzo, less fluorescent on dark UI.
for old, new in {
    '#59D8C2': '#46B5A5',
    '#173A37': '#16312F',
    '#6CE4CF': '#56C1B0',
...
# Better default readability for inherited text + DPI-safe layout rounding.
app = replace_once(app,
    '            <Setter Property="FontSize" Value="12"/>\n            <Setter Property="TextOptions.TextFormattingMode" Value="Display"/>',
    '            <Setter Property="FontSize" Value="13"/>\n            <Setter Property="UseLayoutRounding" Value="True"/>\n            <Setter Property="SnapsToDevicePixels" Value="True"/>\n            <Setter Property="TextOptions.TextFormattingMode" Value="Display"/>',
    'global window readability')

# Shared typography / controls.  Larger hit boxes also stop the clipped/stacked
# appearance caused by 24-28 DIP controls inside fixed-height rows at 125/150% DPI.
replacements = [
...
# -----------------------------------------------------------------------------
# MainWindow: slightly larger small text, a few more DIPs around dense controls,
# and matching chrome dimensions.  Titles stay compact; only tiny text grows.
# -----------------------------------------------------------------------------
...
# Fixed-height compact rows reported as visually overlapping on the screenshot.
x = x.replace('<Grid.RowDefinitions><RowDefinition Height="40"/><RowDefinition Height="*"/></Grid.RowDefinitions>',
              '<Grid.RowDefinitions><RowDefinition Height="46"/><RowDefinition Height="*"/></Grid.RowDefinitions>')
x = x.replace('<Border Style="{StaticResource CompactListItemBorder}" MinHeight="54">',
              '<Border Style="{StaticResource CompactListItemBorder}" MinHeight="62">')
x = x.replace('<Grid.RowDefinitions><RowDefinition Height="88"/><RowDefinition Height="*"/></Grid.RowDefinitions>',
              '<Grid.RowDefinitions><RowDefinition Height="96"/><RowDefinition Height="*"/></Grid.RowDefinitions>', 1)

# Slightly increase only small explicit type.  WPF sizes are DIPs, so this works
# consistently on 100/125/150% scaling while preserving 14/15/20pt headings.
def bump_font(match):
    raw = match.group(1)
    try:
        v = float(raw)
...
# Keep the splash/current-version fallback coherent.
...
# R34 crash report labels are useful for old crashes, but new reports generated by
# R35 should identify the current production build.
...
# R34 feedback diagnostics also need the new version label.
...
# Version stamp all projects so production payload cannot mix R34/R35 DLLs.
...
notes = {
    ...
    'added':[
        'DPI-safe layout rounding и pixel snapping для более ровных границ и текста при 125/150% масштабировании Windows.',
        'Увеличены самые мелкие подписи, статусы, категории и кнопки; основная навигация теперь наследует 13 DIP.'
    ],
    'changed':[
        'Яркий бирюзовый акцент заменён более спокойным фирменным teal; hover также стал мягче.',
        'Compact/Table/Preset кнопки получили больше высоты и внутреннего воздуха, чтобы не выглядеть наложенными или обрезанными.',
        'Заголовочные строки Выбранное/Ход работы и карточки списка получили дополнительную высоту.',
        'Левая панель расширена на 12 DIP, верхняя строка — на 4 DIP, чтобы увеличенный текст не зажимался.'
    ],
    'fixed':[
        'Устранена основная причина визуального наложения кнопок в плотных фиксированных строках на 2K/масштабированном рабочем столе.',
        'Сохранены Process Reduction R34, Feedback/Crash Reporter, Update Center, Snapshot/Undo и R33 runtime stability gates.'
    ]
}
